...
///
/// Args (same as `bin/bump_version.dart`):
///   [--pubspec <path>]  explicit pubspec path (highest priority)
///   [--build-only]      only bump the build number
///   [<msg-file>]        commit message file (for major bump hints)
///
/// Path resolution: explicit path > `app/pubspec.yaml` (monorepo) > `<cwd>/pubspec.yaml`.
/// Returns the process exit code (0 = success).
Future<int> runBump(List<String> args) async {
  var buildOnly = false;
  String? pubspecArg;
  String? msgFile;
  for (final a in args) {
...
String? _resolvePubspec(String? explicit) {
  ...
  final inApp = File(
      '${cwd.path}${Platform.pathSeparator}app${Platform.pathSeparator}pubspec.yaml');
